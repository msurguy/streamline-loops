...
export const PRESETS: Record<string, Preset> = {
  ...
  // ——— top-down presets: 2D fields in the xz-plane (vy = 0), best viewed
  // from above — pair with the eased top-down camera and/or orthographic ———
  'Star Mandala': {
    // radial petal flow (6-fold) + constant swirl → concentric wavy star rings
    vx: '(x*0.7*cos(6*atan2(z,x)) - z)/max(0.15, length(x,z))',
    vy: '0',
    vz: '(z*0.7*cos(6*atan2(z,x)) + x)/max(0.15, length(x,z))',
    seedCount: 1500,
    noiseFrequency: 1.6,
    fieldScale: 1,
    region: 'disc',
  ...
  'Wave Interference': {
    // lines follow contours of two overlapping ripple sources at x = ±0.7
    vx: '-(cos(5*length(x-0.7,z))*z/max(0.1,length(x-0.7,z)) + cos(5*length(x+0.7,z))*z/max(0.1,length(x+0.7,z)))',
    vy: '0',
    vz: 'cos(5*length(x-0.7,z))*(x-0.7)/max(0.1,length(x-0.7,z)) + cos(5*length(x+0.7,z))*(x+0.7)/max(0.1,length(x+0.7,z))',
    seedCount: 1500,
    noiseFrequency: 1.6,
    fieldScale: 1,
    region: 'disc',
  ...
  'Whirlpool': {
    // logarithmic spirals; the 0.35 outward drift sets how tightly they wind
    vx: '(-z + 0.35*x)/max(0.15, length(x,z))',
    vy: '0',
    vz: '(x + 0.35*z)/max(0.15, length(x,z))',
    seedCount: 1500,
    noiseFrequency: 1.6,
    fieldScale: 1,
    region: 'disc',
  ...
  'Flat Curl': {
    // y frozen to 0 → the whole slab shares one 2D curl field, so the
    // pattern stays crisp viewed from above
    vx: 'curlX(x*freq, 0, z*freq)',
    vy: '0',
    vz: 'curlZ(x*freq, 0, z*freq)',
    seedCount: 1500,
    noiseFrequency: 1.6,
    fieldScale: 1,
    region: 'disc',
  ...
  'Vortex Quilt': {
    // stream function sin(kx)·sin(kz): checkerboard of counter-rotating tiles
    vx: 'sin(x*4)*cos(z*4)',
    vy: '0',
    vz: '-cos(x*4)*sin(z*4)',
    seedCount: 1500,
  ...
  'Tree Rings': {
    // concentric circles with organic radial wobble from noise
    vx: '-z/max(0.2,length(x,z)) + 0.25*noise(x*2, 0, z*2)*x/max(0.2,length(x,z))',
    vy: '0',
    vz: 'x/max(0.2,length(x,z)) + 0.25*noise(x*2, 0, z*2)*z/max(0.2,length(x,z))',
    seedCount: 1500,
    noiseFrequency: 1.6,
    fieldScale: 1,
    region: 'disc',
